...
//  Name              CanId     ReadId          WriteId         ConfirmationID
  ...
  { "FET"           , 0x202  ,  {0x41, 0x01},   {0x40, 0x01},   {0x00, 0x00}}
};
...
const ElsterIndex *  processCanMessage(unsigned short can_id, std::string &signalValue, std::vector<unsigned char> msg)
{
    ...
    ESP_LOGI("processCanMessage()", "%d:\t%s:\t%s\t(%s):%x:%x:%x:%x:%x:%x:%x:%x", can_id, ei->Name, charValue, ElsterTypeStr[ei->Type], msg[0],msg[1],msg[2],msg[3],msg[4],msg[5],msg[6],msg[7]);
...
void update_COP_WW_today()
{
    id(COP_WW_today).publish_state((id(WAERMEERTRAG_WW_TAG).state / id(EL_AUFNAHMELEISTUNG_WW_TAG).state));
    return;
}
void update_COP_HEIZ_today()
{
    id(COP_HEIZ_today).publish_state((id(WAERMEERTRAG_HEIZ_TAG).state / id(EL_AUFNAHMELEISTUNG_HEIZ_TAG).state));
    return;
}
void update_COP_GESAMT_today()
{
    id(COP_GESAMT_today).publish_state((id(WAERMEERTRAG_WW_TAG).state + id(WAERMEERTRAG_HEIZ_TAG).state) / (id(EL_AUFNAHMELEISTUNG_WW_TAG).state + id(EL_AUFNAHMELEISTUNG_HEIZ_TAG).state));             
    return;
}
